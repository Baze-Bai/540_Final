...
def calculate_metrics(pred_img_path, ground_truth_path):
    """
    Calculate SSIM and PSNR metrics between predicted and ground truth images.
    
    Parameters:
        pred_img_path (str): Path to the predicted image
        ground_truth_path (str): Path to the ground truth image
        
    Returns:
        tuple: (ssim_value, psnr_value) containing the calculated metrics
    """
    # Load images
    pred_img = np.array(Image.open(pred_img_path).convert('L'))
    gt_img = np.array(Image.open(ground_truth_path).convert('L'))
    
    # Ensure images are same size
    if pred_img.shape != gt_img.shape:
        pred_img = np.array(Image.fromarray(pred_img).resize(
            (gt_img.shape[1], gt_img.shape[0]), Image.BILINEAR))
    
    # Normalize images to [0, 255] range if needed
    if pred_img.max() <= 1.0:
